...
async def check_if_chat_loaded(tab: Tab) -> bool:
    try:
        button = await tab.find(css_selector=VOICE_CSS_SELECTOR, timeout=5, raise_exc=False)
...
async def send_request_and_get_response(tab: Tab, message: str):
    ...
    await tab.find(css_selector=VOICE_CSS_SELECTOR, timeout=TIMEOUT_SECONDS)

    await asyncio.sleep(1)  # OpenAI displays the button before the end of the answer.

    # Test if 2 solutions are available.
    prefered_answer_buttons = await tab.query(css_selector=PREFERED_RESPONSE_BUTTON_CSS_SELECTOR)
    if len(prefered_answer_buttons) > 0:
        await prefered_answer_buttons[0].click()
        await asyncio.sleep(1)
